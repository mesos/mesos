...
private:
  ...
  Future<Nothing> __recover(list<hashset<ContainerID> > results);
    ...
    const Option<state::SlaveState>& state)
{
  ...
  foreachkey (Containerizer* containerizer, containers_) {
...
Future<Nothing> ComposingContainerizerProcess::_recover()
{
  ...
  list<Future<hashset<ContainerID> > > futures;
  foreachkey (Containerizer* containerizer, containers_) {
    futures.push_back(containerizer->containers());
  }

  return collect(futures)
    .then(defer(self(), &Self::__recover, lambda::_1));
}


Future<Nothing> ComposingContainerizerProcess::__recover(
    list<hashset<ContainerID> > containers) // Non-const because we mutate.
{
  foreachkey (Containerizer* containerizer, containers_) {
    CHECK(!containers.empty());
    containers_[containerizer] = containers.front();
