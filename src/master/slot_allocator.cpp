...
SlotAllocator::SlotAllocator(Master* master_)
  : master(master_)
{
  slotsPerNode = master->getConf().get<int>("slots_per_node", 2);
  LOG(INFO) << "Creating SlotAllocator, slotsPerNode = " << slotsPerNode;
}


void SlotAllocator::frameworkAdded(Framework* framework)
{
...
void SlotAllocator::frameworkRemoved(Framework* framework)
{
...
void SlotAllocator::slaveAdded(Slave* slave)
{
...
void SlotAllocator::slaveRemoved(Slave* slave)
{
...
void SlotAllocator::taskRemoved(Task* task, TaskRemovalReason reason)
{
...
void SlotAllocator::offerReturned(SlotOffer* offer,
...
void SlotAllocator::offersRevived(Framework* framework)
{
...
vector<Framework*> SlotAllocator::getAllocationOrdering()
{
...
void SlotAllocator::makeNewOffers()
{
...
void SlotAllocator::makeNewOffers(Slave* slave)
{
  vector<Slave*> slaves;
...
void SlotAllocator::makeNewOffers(const vector<Slave*>& slaves)
{
