...
jobject convert(JNIEnv* env, const Log::Position& position)
{
  ...
  uint64_t value =
    ((uint64_t) (identity.c_str()[0] & 0xff) << 56) |
    ((uint64_t) (identity.c_str()[1] & 0xff) << 48) |
    ((uint64_t) (identity.c_str()[2] & 0xff) << 40) |
    ((uint64_t) (identity.c_str()[3] & 0xff) << 32) |
    ((uint64_t) (identity.c_str()[4] & 0xff) << 24) |
    ((uint64_t) (identity.c_str()[5] & 0xff) << 16) |
    ((uint64_t) (identity.c_str()[6] & 0xff) << 8) |
    ((uint64_t) (identity.c_str()[7] & 0xff));
  ...
  jlong jvalue = value; // Java is expecting a long (i.e., signed).
   ...
   jobject jpids)
{
  ...
  while (env->CallBooleanMethod(jiterator, hasNext)) {
    ...
    const char* temp = env->GetStringUTFChars(js, NULL);
    if (temp == NULL) {
      return; // OutOfMemoryError already thrown.
    }
    std::string s(temp);
   ...
   jstring jznode)
{
  ...
  temp = env->GetStringUTFChars(jpath, NULL);
  ...
  temp = env->GetStringUTFChars(jservers, NULL);

  if (temp == NULL) {
    return; // OutOfMemoryError already thrown.
  }

  std::string servers(temp);

  env->ReleaseStringUTFChars(jservers, temp);
  ...
  temp = env->GetStringUTFChars(jznode, NULL);

  if (temp == NULL) {
    return; // OutOfMemoryError already thrown.
  }

  std::string znode(temp);

  env->ReleaseStringUTFChars(jznode, temp);
